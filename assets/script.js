...
var API_URL = "http://vam-imagewall.appspot.com/api/v1/get_images.json?dt=",
    image_array = [],
    array_index = 0;
...
$(document).ready(function(){
    ...
    var call_api = setInterval("ping_the_api()", 500000);

    // 3: Set up a request to get the latest item from top of the pile
    var update_lead = setInterval("update_lead_image()", 15000);

});

function ping_the_api(){
    ...
    var url = API_URL + date;

    // 2: Ping the API
    $.getJSON(url, function(images) {
        // 3: Add results to global image array
        $.each(images, function(i,data){
            image_array.push(data);
        });

        // 4: Update data-lastfetch with now.
